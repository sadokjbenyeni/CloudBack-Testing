...
namespace CloudBacktesting.PaymentService.Infra.Tests.Security
{
    [TestFixture]
    public class CloudBacktestingAuthenticationHandlerTests
    {
        ...
        public async Task Should_authenticate_user_when_token_is_valid()
        {
            var user = new UserIdentity()
            {
                ...
                Additionals = new Dictionary<string, string>() { { "paymentaccountid", "id" } },
            };
        ...
        public async Task Should_have_claims_when_token_is_valid()
        {
            ...
            Assert.That(result.Principal.HasClaim(claim => string.Equals(claim.Type, "paymentaccountid")), Is.True);
        }
    }
    ...
    public class HeaderDictionaryTests : Dictionary<string, StringValues>, IHeaderDictionary
    {
        ...
        public HeaderDictionaryTests(IDictionary<string, StringValues> dictionary) : base(dictionary) {}
