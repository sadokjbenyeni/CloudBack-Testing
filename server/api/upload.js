...
const fonts = {
  Roboto: {
    normal: '/var/www/histodataweb/node_modules/pdfmake/font/Roboto-Regular.ttf',
    bold: '/var/www/histodataweb/node_modules/pdfmake/font/Roboto-Medium.ttf',
...
function tabSum(serviceTotal, vatTotal, invoiceTotal, currency) {
  return {
    table: {
      widths: ['33%', '55%', '10%'],
      body: [
...
function qhAddress(address, cp, city, country, sasu, rcs, vat) {
  return {
    text: address + '\n' + cp + ' ' + city + '\n' + country + '\n' + sasu + '\n' + rcs + '\n' + vat,
    style: 'invoiceBillingAddress'
  };
...
const pdfOrderFrom = multer.diskStorage({
  destination: "./command",
  filename: function (req, file, cb) {
    cb(null, file.originalname.replace(/ /g, ""));
  }
...
function link(cmd) {
  ...
  footer = {
    table: {
      ...
      widths: ['50%', '50%'],
      body: [
        productMessage('test message product'),
        [
          {
          ...
          },
          {
            ...
            table: {
              widths: ['98%'],
              body: [[[tabSum(this.totalHt, this.totalVat, this.totalTTC, this.currency), { text: '\n', fontSize: 8 }, wireTransfer()]]]
            },
            layout: { defaultBorder: false }
          }
        ]
  ...
  content.push(
    {
      columns: [[{ image: logo(), width: 200, height: 60 }, qhAddress('86 boulevard Haussmann', '75009', 'PARIS', 'FRANCE', 'SASU au capital de 48 782 296 €', 'RCS Paris 44970324800053', 'VAT : FR00449703248')], { text: '', width: 150 }],
    },
    // Billing Address
    '\n',
  ...
  return printer.createPdfKitDocument(invoice);
}


let pdfOrderFromUpdt = multer({ storage: pdfOrderFrom });
...
router.get('/linkoo', (req, res, next) => {
